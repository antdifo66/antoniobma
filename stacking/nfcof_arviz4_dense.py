...
def createfit(params,noise,error,index,datarange,seed,N, a_dFix,fixpm,findelpd, myimpsamp, nologsumexp):
    ...
    #s2 = np.square(yerr)+0.25*np.square(genmodel(params,xp))
    #print("s2: "+str(s2))
    #print("s2 type: "+str(type(s2)))
    #cov = np.diag(np.square(yerr)+0.25*np.square(genmodel(params,xp)))
    fullerr = 8*(noise+error)
    cov = np.zeros((len(yerr),len(yerr)))
    cdiag = fullerr*np.ones(len(yerr))
    cdiagoff = fullerr*0.5*np.ones(len(yerr)-1)
    cov+=np.diag(cdiag)
    cov+=np.diagflat(cdiagoff,1)
    cov+=np.diagflat(cdiagoff,-1)
    #print("cov: "+str(cov))
...
def runarviz(ardatacubiclow,ardatacubicfid,ardatacubichigh):
    ''' '''
    arvizoutstacking, log_weights = az.compare({"cubicfid":ardatacubicfid,"cubichigh":ardatacubichigh, "cubiclow":ardatacubiclow}, ic='loo',method='stacking')
    az.plot_compare(arvizoutstacking)

    arvizoutpBMA, log_weights = az.compare({"cubicfid":ardatacubicfid,"cubichigh":ardatacubichigh, "cubiclow":ardatacubiclow}, ic='loo',method='pseudo-BMA')
    az.plot_compare(arvizoutpBMA)


    return arvizoutpBMA['weight'], arvizoutstacking['weight'], log_weights

def calcweights_mine(elpd, expPLLs):
    ...
    stackweightsoln = minimize(nll,winit,args=(np.shape(expPLLs)[1],3,expPLLs),
                               bounds=bounds,
                               constraints=cons)
...
def runbma(params):
    '''
    This function uses Simone Paradiso's FastMPC BMA code to average the MCMC chain samples between the generated models.

    Parameters:
    -----------
    params: float array, true values of genmodel parameters

    Returns:
    --------
    MCbmasamples: MCSample object containing BMA info
    '''
    #don't forget to insert '#' within each txt file before running
    roots = ["\chainar2cubiclowfix", "\chainar2cubicfidfix", "\chainar2cubichighfix"] # this might not be correct
    models = ["cubiclow","cubicfid","cubichigh"]
    temperature = {'cubiclow': 1,"cubicfid": 1, 'cubichigh': 1}
    model_prior = {'cubiclow': 0.3, 'cubicfid': 0.3, 'cubichigh': 0.3}
    parameters = {'cubiclow':['a','b','c'], 'cubicfid':['a','b','c'],'cubichigh':['a','b','c']}        #these have to match the parameter names in the .txt
    burnin = [0.3,0.3,0.3]
    chainpath = "C:/Users/zombk/Documents/vscode/.venv/W2025coopcode/.venv/code_for_getting_started/arviz_stuff/nonfactcase"
...
def runpseudobma(params,pbmaweights, ifPandas):
    '''
    This function uses Simone Paradiso's FastMPC BMA code to average the MCMC chain samples between the generated models
    but now weighted with pseudo-BMA weights from arviz.
    ...
    '''
    if ifPandas:
        pbmaweight_vals=pbmaweights.to_dict()
    else:
        pbmaweight_vals=pbmaweights
    print("pbma")
    print(pbmaweight_vals)
    ...
    out = fmpc.runpseudobma(pbmaweight_vals,roots=roots, models=models, chainDir=chainpath, burnin=burnin,
               priors=model_prior, temperature=temperature, pars=parameters, estimator='learnt harmonic')
...
def runstacking(params,stackweights,ifPandas):
    '''
    This function uses Simone Paradiso's FastMPC BMA code to average the MCMC chain samples between the generated models
    but now weighted with stacking weights from arviz.
    ...
    '''
    if ifPandas:
        stackweight_vals = stackweights.to_dict()
    else:
        stackweight_vals = stackweights
    print("stack")
    print(stackweight_vals)
    ...
    out = fmpc.runstack(stackweight_vals,roots=roots, models=models, chainDir=chainpath, burnin=burnin,
               priors=model_prior, temperature=temperature, pars=parameters, estimator='learnt harmonic')
...
def plotbma(params, gdsamplescubic, gdsamplespara, gdsamplesquart, MCbmasamples):

    g = plots.get_subplot_plotter()
    g.triangle_plot([gdsamplescubic, gdsamplespara,  gdsamplesquart, MCbmasamples], filled=False, legend_labels=['Cubic-Low','Cubic-Fid','Cubic-High','BMA'], 
                markers={'a':params[2],'b':params[1],'c':params[0]}, line_args=[{'color':'orange'},
                                                                                              {'color':'red'},
                                                                                              {'color':'blue'},
...
def plotmethods(params, MCbmasamples, mcpbma2samps, mcstack2samps,mcpbma3samps, mcstack3samps):
    g = plots.get_subplot_plotter()
    g.triangle_plot([MCbmasamples, mcpbma2samps, mcstack2samps,mcpbma3samps, mcstack3samps], filled=False, 
                    legend_labels=['BMA','Pseudo-BMA','Stacking','P-BMA Full', 'Stack Full'], 
                markers={'a':params[2],'b':params[1],'c':params[0]}, line_args=[
                                                                                              {'color':'green','lw':2},
                                                                                              {'color':'red','ls':'--','lw':1.75},
                                                                                              {'color':'blue','ls':'--','lw':1.5},
                                                                                              {'color':'cyan','ls':'--','lw':1.25},
                                                                                              {'color':'orange','ls':'--','lw':1.1}], 
                                                                                              contour_colors=['green','red','blue','cyan', 'orange'])
    
    h = plots.get_subplot_plotter()
    h.triangle_plot([mcpbma2samps,mcpbma3samps], filled=False, 
                    legend_labels=['My ImpSamp, Arviz Weight','Mine Both'], 
                markers={'a':params[2],'b':params[1],'c':params[0]}, line_args=[
                                                                                              {'color':'cyan','ls':'--','lw':2.5},
                                                                                              {'color':'red','ls':'--','lw':2.0}],
                                                                                              contour_colors=['cyan','red'])
    
    i = plots.get_subplot_plotter()
    i.triangle_plot([mcstack2samps,mcstack3samps], filled=False, 
                    legend_labels=['My ImpSamp, Arviz Weight','Mine Both'], 
                markers={'a':params[2],'b':params[1],'c':params[0]}, line_args=[
                                                                                              {'color':'cyan','ls':'--','lw':2.5},
                                                                                              {'color':'red','ls':'--','lw':2.0}], 
